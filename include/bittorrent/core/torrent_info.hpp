...
public:
    ...
    std::int64_t piece_length() const noexcept {
        return piece_length_;
    }

    std::size_t piece_count() const noexcept {
        return piece_hashes_.size();
    }

    const std::vector<SHA1Hash>& piece_hashes() const noexcept {
        return piece_hashes_;
    }

    const InfoHash& info_hash() const noexcept {
        return info_hash_;
    }

    const std::string& announce() const noexcept {
        return announce_;
    }

    const std::vector<std::vector<std::string>>& announce_list() const noexcept {
    ...
    const std::vector<FileInfo>& files() const noexcept {
        return files_;
    }
    ...
    const std::optional<std::string>& created_by() const noexcept {
        return created_by_;
    }
